...
- **Prisma ORM** + **PostgreSQL** (Neon) — dùng chung một database cho cả local và production
...
```bash
npm install
cp .env.example .env      # điền DATABASE_URL (Neon) và các biến khác
npx prisma generate
npm run dev
```

Database đã được tạo sẵn trên Neon và nạp dữ liệu mẫu — không cần chạy lại
`migrate`/`seed` trừ khi bạn muốn dùng một database Neon khác. Nếu vậy, chạy:

```bash
npx prisma migrate deploy   # tạo bảng trên database mới
npm run db:seed             # nạp lại dữ liệu sản phẩm mẫu
```
...
Vì local và Vercel dùng chung một database Neon, `npx prisma studio` chạy từ
máy bạn sẽ luôn thao tác trên đúng dữ liệu mà website production đang hiển thị.
...
Database PostgreSQL (Neon) đã được tạo và nạp dữ liệu sẵn — chỉ còn kết nối
Vercel với repo GitHub và khai báo lại các biến môi trường.
